...
def read_files_and_store_locally(
    ...
    video_list_s3_original_video = aux.load_s3(
        bucket=bucket_name, prefix=prefix_orginal_files
    )
    video_list_s3_reduced_video = aux.load_s3(
        bucket=bucket_name, prefix=prefix_reduced_files
    )
...
def match_files(original_folder, modified_folder):
    ...
    for i in range(len(os.listdir(original_folder))):
        original_video_path = os.path.join(
            original_folder, os.listdir(original_folder)[i]
        )
