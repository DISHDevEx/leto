...
class Basic(torch.nn.Module):
    """
    Basic module for neural network building.

    Args:
        strType (str): Type of the module.
        intChans (List[int]): List of input channel sizes.
        objScratch (Optional[Dict]): Optional dictionary for scratch storage.

    Attributes:
        strType (str): Type of the module.
        netEvenize: Evenize network for padding.
        netMain: Main network.
        netShortcut: Shortcut network.
    """
        ...
        for intPart, strPart in enumerate(self.strType.split("+")[0].split("-")):
            if strPart.startswith("evenize") == True and intPart == 0:
                ...
                class Up(torch.nn.Module):
                    ...
                    def forward(self, tenIn: torch.Tensor) -> torch.Tensor:
                        """
                        Forward pass of the Basic module.

                        Args:
                            tenIn (torch.Tensor): Input tensor.
...
class Network(torch.nn.Module):
    """
    Network module for neural network building.

    Attributes:
        netExtractor: Extractor network.
        netFiv: Decoder network for level 5.
        netFou: Decoder network for level 4.
        netThr: Decoder network for level 3.
        netTwo: Decoder network for level 2.
        netOne: Decoder network for level 1.
    """
        ...
        class Decoder(torch.nn.Module):
            ...
            def forward(self, tenOne, tenTwo, tenFlow):
                ...
                if tenFlow is None:
                    ...
                    tenMain.append(
                        self.netCostacti(costvol.costvol_func.apply(tenOne, tenTwo))
                    )
                    ...
                    tenMain.append(
                        self.netCostacti(
                            costvol.costvol_func.apply(
                                tenOne, backwarp.backwarp(tenTwo, tenFlow.detach())
                            )
                        )
                    )
    ...
    def bidir(self, tenOne, tenTwo):
        """
        Bidirectional forward pass of the Network module.

        Args:
            tenOne (torch.Tensor): Input tensor for frame 1.
            tenTwo (torch.Tensor): Input tensor for frame 2.

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: Forward and backward flow tensors.
        """
